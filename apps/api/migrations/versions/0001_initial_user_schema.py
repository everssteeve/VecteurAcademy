...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "role", sa.String(50), nullable=False, server_default=sa.text("'learner'")
        ),
        sa.Column(
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "user_id", "module_id", name="uq_module_progress_user_module"
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "answered_at", sa.DateTime(timezone=True), server_default=sa.text("NOW()")
        ),
        sa.CheckConstraint(
