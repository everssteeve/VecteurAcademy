...
async def get_progress(
    user_id: uuid.UUID, session: AsyncSession = Depends(get_session)  # noqa: B008
) -> ProgressRead:
...
async def track_module_start(
    body: ModuleStartRequest, session: AsyncSession = Depends(get_session)  # noqa: B008
) -> dict:
...
async def save_quiz_result(
    body: QuizResultWithUser, session: AsyncSession = Depends(get_session)  # noqa: B008
) -> QuizResultRead:
