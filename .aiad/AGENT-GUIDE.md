...
| 2026-04-28 | Tests Playwright logout flaky en `fullyParallel: true` : plusieurs tests logout partagent le même JWT (storageState), les POST simultanés au serveur Turbopack de dev créent une interférence — le test "après déconnexion, /dashboard → /login" échoue de façon non déterministe | Ajouter `test.describe.configure({ mode: "serial" })` au describe parent contenant tous les tests logout. Cela ne ralentit que ce bloc (tous les autres tests restent parallèles). | Flaky test — 1 failure sur 5 en parallèle, 0/5 en serial |
| 2026-04-29 | FastAPI `Annotated[str, Header()]` (header requis) retourne HTTP 422 (Unprocessable Entity) si le header est absent — pas 401. SPEC-014 spécifiait 401 sans token → les tests échouaient avec 422. | Rendre le header optionnel : `Annotated[str \| None, Header()] = None`, puis lever 401 explicitement si `None`. Pattern standard pour tout header d'auth FastAPI qui doit retourner 401 (RFC 7235) et non 422. | 3 tests 401 en échec au premier run, corrigés après diagnostic |
| 2026-04-29 | `test_auth.py` — fixture `_VALID_USER` sans `first_name`/`last_name` — migration `0002_add_first_last_name_to_users` les a rendus requis mais `test_auth.py` n'a pas été mis à jour → 3 tests cassés en permanence depuis l'itération 3 | Après toute migration ajoutant un champ requis, passer en revue TOUS les tests qui utilisent `POST /auth/register` et mettre à jour leurs fixtures. Ne pas laisser des tests d'authentification cassés en permanence — ils masquent de vraies régressions. | Tests auth inutilisables — drift silencieux depuis 2026-04-28 |
...
| 2026-04-28 | Auth.js v5 (`next-auth@5.0.0-beta.31`) — `session.user.id` absent du runtime malgré `authorize()` retournant `{ id }` | Auth.js v5 beta.31 construit l'objet session avec `{ name, email, image }` uniquement avant d'appeler le callback `session`. Le callback doit explicitement copier `token.sub` : `session.user.id = token.sub as string`. Sans cette ligne, `session.user.id === undefined` partout (dashboard, server actions) → données jamais persistées. | Bloquant fonctionnel — toute la persistance progression et quiz était silencieusement désactivée |
| 2026-04-28 | SPEC-013 — DoOD spécifiait `page.route()` pour mocker `GET /progress` dans les tests Playwright | `page.route()` intercepte uniquement les requêtes HTTP émises par le navigateur. Les `fetch()` exécutés dans un Next.js Server Component s'exécutent côté Node.js et contournent entièrement Playwright — le mock est silencieusement ignoré. | Pour les Server Components, utiliser la fixture `request` Playwright pour POST/seeder des données réelles via l'API, ou faire des assertions sur les données réelles déjà en DB. Documenter cette contrainte dans le DoOD dès la rédaction de la SPEC. |
